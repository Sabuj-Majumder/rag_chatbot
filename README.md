...
### 1. 📂 Content Ingestion (Knowledge Vault)
- Open the sidebar using the arrow icon (if collapsed).
- Click **"Secure Upload"** to select files.
- Supported Formats: **PDF, DOCX, TXT, CSV, SQL, PNG, JPG**.
- Click **"📥 Secure Ingest"** to process them. You'll see "Encrypting & Indexing" status.
- *Tip: Once ingested, documents are permanently stored in the local vector database.*

### 2. 💬 Interactive Chat
- Type your question in the main chat input at the bottom.
- The AI will search your ingested documents for relevant context.
- **Tabs Explained**:
    - **Analysis**: The direct answer generated by Llama 3.
    - **Context**: The exact text snippets retrieved from your documents.
    - **Audit Trail**: Metadata showing which files were used (Filename, Source Type).

### 3. 👁️ Visual Intelligence
- Use the **"Visual Intelligence"** uploader in the sidebar.
- Upload an image (e.g., a diagram, chart, or screenshot).
- Click **"Analyze Visual"**.
- The AI will analyze the image content (using OCR and Vision capabilities) and answer questions about it.

### 4. 🧹 Session Management
- Click **"🗑️ Delete History"** in the sidebar to clear the current chat session.
